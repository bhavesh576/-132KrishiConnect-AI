...
interface Txn {
  txn_id: number; crop: string; grade: string; buyer_name: string;
  final_price_per_qtl: number; final_qty_tonnes: number; order_value: number;
  truck_id: string; logistics_status: string; payment_status: string;
  stage_index: number; stages: string[]; created_at: string;
}
...
function TransactionsInner() {
  ...
  const load = () => apiGet<Txn[]>(`/transactions/farmer/${session.user_id}`).then(setTxns);
  useEffect(() => { load().catch(console.error); }, [session.user_id]);

  const advance = async (x: Txn) => {
    // demo-only convenience to move the prototype stepper without DB edits
    ...
    <div className="mx-auto max-w-7xl px-4 pb-8">
      ...
      <section className="kc-card mt-4 p-4">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px]" data-testid="txn-table">
            <thead><tr>
              <th className="kc-th">{t("labels.lot")}</th><th className="kc-th">{t("labels.crop")}</th>
              <th className="kc-th">{t("labels.buyer")}</th><th className="kc-th">{t("labels.qty")}</th>
            ...
            <tbody>
              {txns.map((x) => (
                <>
                  <tr key={x.txn_id}>
                    ...
                    <td className="kc-td">{x.buyer_name}</td>
              ...
              {!txns.length && <tr><td className="kc-td text-center text-muted" colSpan={9}>{t("labels.noData")}</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
...
export default function Page() {
  return <RoleGuard role="FARMER"><TransactionsInner /></RoleGuard>;
}
